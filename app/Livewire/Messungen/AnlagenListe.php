    ...
    <div class="row g-2 mb-3">
        ...
        <div class="col-4 col-md-4">
            <div class="card border-success h-100 stat-card cursor-pointer" 
                 wire:click="$set('filterGemessen', '1')"
                 role="button">
                <div class="card-body text-center py-2">
        ...
        <div class="col-4 col-md-4">
            <div class="card border-danger h-100 stat-card cursor-pointer"
                 wire:click="$set('filterGemessen', '0')"
                 role="button">
                <div class="card-body text-center py-2">
            ...
            <div class="table-responsive d-none d-lg-block">
                <table class="table table-hover align-middle mb-0" id="anlagenTable">
                    <thead class="table-dark">
                        <tr>
                            <th style="width: 90px;" class="cursor-pointer" wire:click="sortBy('Feld_a')">
                                Kodex
                                @if($sortField === 'Feld_a')
                                    <i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ms-1"></i>
                                @endif
                            </th>
                            <th class="cursor-pointer" wire:click="sortBy('Feld_w')">
                                Beschreibung
                                @if($sortField === 'Feld_w')
                                    <i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ms-1"></i>
                                @endif
                            </th>
                            <th class="cursor-pointer" wire:click="sortBy('Feld_i')">
                                Ort
                                @if($sortField === 'Feld_i')
                                    <i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ms-1"></i>
                                @endif
                            </th>
                            <th class="cursor-pointer" wire:click="sortBy('Feld_k')">
                                Straße
                                @if($sortField === 'Feld_k')
                                    <i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ms-1"></i>
                                @endif
                            ...
                            <th class="cursor-pointer" wire:click="sortBy('Feld_y')">
                                Hersteller
                                @if($sortField === 'Feld_y')
                                    <i class="bi bi-arrow-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ms-1"></i>
                                @endif
                        ...
                        @foreach($anlagen as $anlage)
                            ...
                            <tr class="{{ !$hatMessung ? 'table-danger bg-opacity-25' : '' }}">
                                ...
                                <td>
                                    <a href="{{ route('messungen.anlagen.edit', $anlage->Feld_a) }}" 
                                       class="text-decoration-none text-dark">
                                        {{ Str::limit($anlage->Feld_w, 40) ?: '(keine Beschreibung)' }}
                                    </a>
                                </td>
                                <td>{{ $anlage->Feld_i }}</td>
                                ...
                                <td class="text-center">
                                    @if($hatMessung)
                                        <span class="badge bg-success" title="Messung vorhanden">
                                            <i class="bi bi-check-lg"></i>
                                        </span>
                                    @else
                                        <span class="badge bg-danger" title="Keine Messung {{ $filterJahr }}">
                                            <i class="bi bi-x-lg"></i>
                                        </span>
                                    @endif
                                </td>
...
<style>
    ...
    @media print {
        .btn, #filterCollapse, .card-header[data-bs-toggle], .pagination { display: none !important; }
